...
entity = 'member_his'
src_entity = 'tm_member_his'
DAG_NAME = 'tm_member_his_dag'
...
def process_fileload(is_encrypted = False, is_compressed = False, **kwargs):
    ...
    # if src_filename.find('.') >= 0:
    #     src_filename_no_ext = src_filename[src_filename.find('.') + 1:] +'.csv'
    # else:
    #     src_filename_no_ext = src_filename + '.csv'
# if it's excel, then convert to csv 
    src_file_path = os.path.join(ok_dir_path, src_filename)
    # target_file_path = os.path.join(ok_dir_path, src_filename_no_ext)
    # if( src_file_path.split('.')[-1].lower() in ('xlsx', 'xls') ):
    #     myutil.read_excel_file(src_file_path, target_file_path, skiprow=0 ,keephead=False)
...
def post_process_fileload( **kwargs):
    ...
    OK_FILE_PATH  = kwargs.get('dag_run').conf.get('ok_file_path')
    # if( OK_FILE_PATH.split('.')[-1].lower() in ('xlsx', 'xls') ):
    #     OK_FILE_PATH = OK_FILE_PATH.replace('.xlsx','.csv').replace('.xls','.csv')
    myutil.modify_ok_file_prefix("running", "done", OK_FILE_PATH)
    # os.remove(kwargs.get('dag_run').conf.get('ok_file_path'))
...
preprocess_tm_member_his_task = PythonOperator(
    task_id = 'preprocess_tm_member_his_task',
...
tm_member_his_src2stg_task = PythonOperator(
    task_id='tm_member_his_src2stg_task',
    provide_context = True,
...
tm_member_his_stg2ods_task = PythonOperator(
    task_id='tm_member_his_stg2ods_task',
    provide_context = True,
...
tm_member_his_ods2edw_task = PythonOperator(
    task_id='tm_member_his_ods2edw_task',
    provide_context = True,
...
postprocess_tm_member_his_task = PythonOperator(
    task_id = 'postprocess_tm_member_his_task',
    provide_context = True,
...
preprocess_tm_member_his_task >> tm_member_his_src2stg_task >> tm_member_his_stg2ods_task >> tm_member_his_ods2edw_task >> tm_member_his_updated_1st_purchase_date >>  postprocess_tm_member_his_task