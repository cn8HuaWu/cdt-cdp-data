...
class Src2stgHandler:
    def __init__(self, 
        ...
        self.work_data_dir = self.myutil-test.get_conf('ETL', 'WORK_DATA_DIR')
    ...
    def process_src_data(self):
        ...
        bucket = self.myutil-test.get_oss_bucket()
        ...
        self.myutil-test.upload_local_oss(bucket, self.ok_file_path, backup_path)
        ...
        self.myutil-test.upload_local_oss(bucket, datafile_path, backup_path)
        ...
        # sftp_path = self.myutil-test.get_conf('ETL', 'SFTP_DATA_DIR')
        ...
        if( src_file_path.split('.')[-1].lower() == 'gz' ):
            self.myutil-test.uncompress_gz_file(src_file_path, target_file_path)
        elif( src_file_path.split('.')[-1].lower() in ('xlsx', 'xls') ):
            self.myutil-test.read_excel_file(src_file_path, target_file_path, skiprow=self.excel_skip_row ,keephead=self.has_head, sheetname_in= self.sheetname)
        else:
            shutil.copyfile( src_file_path, os.path.join(entity_data_dir, target_file_path) )
        
        if ( self.src_entity ):
            sql_dict =  self.myutil-test.get_sql_yml_fd(self.src_entity)
        ...
        if ( self.src_entity and self.is_encrypted 
            ...
            self.myutil-test.decrypt_csv_fields(target_file_path,
                target_file_path,
                0, 
                encrypt_col_list, 
                columns_list, 
        ...
        if (self.src_entity 
            ...
            dl_aes_key = self.myutil-test.get_dl_aes_key()
            dl_aes_iv = self.myutil-test.get_dl_aes_iv()
            self.myutil-test.encrypt_csv_fields(target_file_path, target_file_path,0, to_encrypt_list, columns_list, dl_aes_key, dl_aes_iv, del_src= True, keepheader=self.has_head)
        ...
        # self.myutil-test.upload_local_oss(bucket, target_file_path, backup_path)
        
        ## Step 5: Upload Data to OSS Staging
        basename = self.entity_name + "_"+ self.table_suffix +".csv" 
        stg_path = '/'.join( (self.level,self.datasource, self.entity_name, self.batch_date, basename) )
        self.myutil-test.upload_local_oss(bucket, target_file_path, stg_path)
