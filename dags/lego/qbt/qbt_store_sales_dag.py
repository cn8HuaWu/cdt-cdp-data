...
entity = 'store_sales'
src_entity = 'qbt_store_sales'
DAG_NAME = 'qbt_store_sales_dag'
TMALL_FLAG = 'tm'
TAOBAO_FLAG = 'tb'
...
def load_ods2edw(**kwargs):
    ...
    #
    ods2edw = Ods2edwHandler(  batch_date, SRC_NAME, entity, pkey,table_prefix, myutil, db, update_type=update_type, has_param=True )
    ods2edw.start()
...
#     if ("skip_load"  in kwargs.get('dag_run').conf 
#         and kwargs.get('dag_run').conf.get("skip_load").upper() == 'Y' ):
#         return "sku_monitor_dummy"
#     else:
#         return "preprocess_sku_monitor_task"
...
preprocess_store_sales_task = PythonOperator(
    task_id = 'preprocess_store_sales_task',
...
store_sales_src2stg_task = PythonOperator(
    task_id='store_sales_src2stg_task',
    provide_context = True,
...
store_sales_stg2ods_task = PythonOperator(
    task_id='store_sales_stg2ods_task',
    provide_context = True,
...
store_sales_ods2edw_task = PythonOperator(
    task_id='store_sales_ods2edw_task',
    provide_context = True,
...
postprocess_store_sales_task = PythonOperator(
    task_id = 'postprocess_store_sales_task',
    provide_context = True,
...
preprocess_store_sales_task >> store_sales_src2stg_task >> store_sales_stg2ods_task >>  store_sales_ods2edw_task  >>  postprocess_store_sales_task
