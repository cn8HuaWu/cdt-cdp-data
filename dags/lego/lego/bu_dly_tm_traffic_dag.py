...
ODS = 'ODS'
TEMP_FOLDER='Temp'
DAG_NAME = 'bu_dly_tm_traffic_dag'
entity = 'dly_tm_traffic'
src_entity = 'bu_dly_tm_traffic'
...
def b_choice(**kwargs):
    if ("skip_load"  in kwargs.get('dag_run').conf 
        and kwargs.get('dag_run').conf.get("skip_load").upper() == 'Y' ):
        return "dly_tm_traffic_dummy"
    else:
        return "preprocess_dly_tm_traffic_task"
...
branch_skip_load_dly_tm_traffic = BranchPythonOperator(
    task_id='branch_skip_load_dly_tm_traffic',
    python_callable= b_choice,
    provide_context = True,
    dag=dag,
)

dly_tm_traffic_dummy = DummyOperator(
    task_id="dly_tm_traffic_dummy",
    dag=dag,
)

preprocess_dly_tm_traffic_task = PythonOperator(
    task_id = 'preprocess_dly_tm_traffic_task',
    provide_context = True,
...
dly_tm_traffic_src2stg_task = PythonOperator(
    task_id='dly_tm_traffic_src2stg_task',
    provide_context = True,
...
dly_tm_traffic_stg2ods_task = PythonOperator(
    task_id='dly_tm_traffic_stg2ods_task',
    provide_context = True,
...
dly_tm_traffic_ods2edw_task = PythonOperator(
    task_id='dly_tm_traffic_ods2edw_task',
    provide_context = True,
...
dly_tm_traffic_update_dl_dly_sales_rpt = PythonOperator(
    task_id='dly_tm_traffic_update_dl_dly_sales_rpt',
    provide_context = True,
    python_callable = update_downstream,
    op_kwargs = {'myutil':myutil, 'gpdb': db, 'sql_file_name':"dl_dly_sales_rpt" , 'sql_section': 'update_by_jd_and_tm_traffic', 'args': args},
    on_failure_callback = dag_failure_handler,
    trigger_rule='none_failed',
    dag=dag,
)

# dly_tm_traffic_update_dl_mly_sales_rpt = PythonOperator(
#     task_id='dly_tm_traffic_update_dl_mly_sales_rpt',
#     provide_context = True,
#     python_callable = update_downstream,
...
dly_tm_traffic_sync_2_rds_task = SubDagOperator(
    task_id='dly_tm_traffic_sync_2_rds_task',
    subdag=sync_subdag(DAG_NAME, 'dly_tm_traffic_sync_2_rds_task', myutil, entity_conf, args, entity),
    default_args=args,
...
postprocess_dly_tm_traffic_task = PythonOperator(
    task_id = 'postprocess_dly_tm_traffic_task',
    provide_context = True,
...
branch_skip_load_dly_tm_traffic >> dly_tm_traffic_dummy >> dly_tm_traffic_update_dl_dly_sales_rpt
branch_skip_load_dly_tm_traffic >> preprocess_dly_tm_traffic_task
preprocess_dly_tm_traffic_task >> dly_tm_traffic_src2stg_task >> dly_tm_traffic_stg2ods_task >> dly_tm_traffic_ods2edw_task 
dly_tm_traffic_ods2edw_task >> dly_tm_traffic_update_dl_dly_sales_rpt >> dly_tm_traffic_sync_2_rds_task >> postprocess_dly_tm_traffic_task
