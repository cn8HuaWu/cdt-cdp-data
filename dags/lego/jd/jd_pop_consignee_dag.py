...
entity = 'pop_consignee'
src_entity = 'jd_pop_consignee'
DAG_NAME = 'jd_pop_consignee_dag'
...
def load_src2stg(**kwargs):
    ...
    OK_FILE_PATH  = kwargs.get('dag_run').conf.get('ok_file_path')
    src_key = myutil.get_conf('JDAPI', 'AES_KEY')
...
def b_choice(**kwargs):
    if ("skip_load"  in kwargs.get('dag_run').conf 
        and kwargs.get('dag_run').conf.get("skip_load").upper() == 'Y' ):
        return "pop_consignee_dtl_dummy"
    else:
        return "preprocess_pop_consignee_task"
...
branch_skip_load_pop_consig = BranchPythonOperator(
    task_id='branch_skip_load_pop_consignee',
    python_callable= b_choice,
    provide_context = True,
    dag=dag,
)

pop_consignee_dtl_dummy = DummyOperator(
    task_id="pop_consignee_dtl_dummy",
    dag=dag,
)

preprocess_pop_consignee_task = PythonOperator(
    task_id = 'preprocess_pop_consignee_task',
    provide_context = True,
...
pop_consignee_src2stg_task = PythonOperator(
    task_id='pop_consignee_src2stg_task',
    provide_context = True,
...
pop_consignee_stg2ods_task = PythonOperator(
    task_id='pop_consignee_stg2ods_task',
    provide_context = True,
...
pop_consignee_ods2edw_task = PythonOperator(
    task_id='pop_consignee_ods2edw_task',
    provide_context = True,
...
pop_consignee_update_pop_order = PythonOperator(
    task_id='pop_consignee_update_pop_order',
    provide_context = True,
...
pop_consignee_sync_2_rds_task = SubDagOperator(
    task_id='pop_consignee_sync_2_rds_task',
    subdag=sync_subdag(DAG_NAME, 'pop_consignee_sync_2_rds_task', myutil, entity_conf, args,entity),
    default_args=args,
...
postprocess_pop_consignee_task = PythonOperator(
    task_id = 'postprocess_pop_consignee_task',
    provide_context = True,
...
branch_skip_load_pop_consig >> pop_consignee_dtl_dummy
branch_skip_load_pop_consig >> preprocess_pop_consignee_task
preprocess_pop_consignee_task >> pop_consignee_src2stg_task >> pop_consignee_stg2ods_task >> pop_consignee_ods2edw_task
pop_consignee_dtl_dummy >> pop_consignee_update_pop_order
pop_consignee_ods2edw_task >> pop_consignee_update_pop_order >> pop_consignee_sync_2_rds_task>>postprocess_pop_consignee_task