...
def load_src2stg(**kwargs):
    ...
    src2stg = Src2stgHandler(STAGING, batch_date, SRC_NAME, entity, stg_suffix, src_filename, myutil, OK_FILE_PATH, excel_fun_list=excel_fun_list, has_head=False, sheetname = 'Floating_DP01, merge = False,**sheet)
...
preprocess_plan_nip_dkk_task = PythonOperator(
    task_id = 'preprocess_plan_nip_dkk_task',
...
plan_nip_dkk_src2stg_task = PythonOperator(
    task_id='plan_nip_dkk_src2stg_task',
    provide_context = True,
...
plan_nip_dkk_stg2ods_task = PythonOperator(
    task_id='plan_nip_dkk_stg2ods_task',
    provide_context = True,
...
edw_lgc_plan_nip_dkk_create = PythonOperator(
    task_id='edw_lgc_plan_nip_dkk_create',
    provide_context=True,
    python_callable=update_downstream,
    op_kwargs={'myutil': myutil, 'gpdb': db, 'sql_file_name': "lgc_plan_nip_dkk",
...
edw_lgc_plan_nip_dkk_delete = PythonOperator(
    task_id='edw_lgc_plan_nip_dkk_delete',
    provide_context=True,
    python_callable=update_downstream,
    op_kwargs={'myutil': myutil, 'gpdb': db, 'sql_file_name': "lgc_plan_nip_dkk",
...
edw_lgc_plan_nip_dkk_insert = PythonOperator(
    task_id='edw_lgc_plan_nip_dkk_insert',
    provide_context=True,
    python_callable=update_downstream,
    op_kwargs={'myutil': myutil, 'gpdb': db, 'sql_file_name': "lgc_plan_nip_dkk",
...
postprocess_plan_nip_dkk_task = PythonOperator(
    task_id = 'postprocess_plan_nip_dkk_task',
    provide_context = True,
