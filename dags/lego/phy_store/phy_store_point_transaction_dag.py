...
entity = 'store_point_transaction'
src_entity = 'phy_store_point_transaction'
DAG_NAME = 'phy_store_point_transaction_dag'
...
api_interval = Variable.get('interval_phy_store_point_transaction').strip()
dag_start_date = Variable.get('dag_start_date').strip()

lego_lewin_mail_to = Variable.get('lego_lewin_mail_to').strip().split(",") 
lego_lewin_subject = Variable.get('lego_lewin_subject').strip() + "- LEWIN member point transaction" 
lego_lewin_html_content =  "LEWIN " + Variable.get('lego_lewin_html_content').strip() + "So far, No new coming data in phy_store_point_transaction topic since last run"
...
def has_new_data(**kwargs):
    updated_flag = kwargs['task_instance'].xcom_pull(key='new_data_flag', task_ids='phy_store_point_transaction_sensor_task')
    if ( updated_flag in('', 'False') ):
        return "phy_store_point_transaction_nodata_mail_task"
    else:
        return "phy_store_point_transaction_stg2ods_task"
...
def load_ods2edw(**kwargs):
    ...
    #
    my_batch_date = kwargs['task_instance'].xcom_pull(key='batch_date', task_ids='phy_store_point_transaction_stg2ods_task')
...
args = {
    ...
    'retries': 0,
    "src_name": SRC_NAME,
    "batch_date_args": 'phy_store_point_transaction_stg2ods_task',
}
...
phy_store_point_transaction_sensor_task = DatahubSensor(
    mydb=db, 
    dh_id=src_entity, 
    myutil= myutil, 
    task_id='phy_store_point_transaction_sensor_task', 
    poke_interval=100, 
    dag=dag
)
...
phy_store_point_transaction_nodata_mail_task = EmailOperator(
    task_id="phy_store_point_transaction_nodata_mail_task",
    dag=dag,
...
phy_store_point_transaction_stg2ods_task = PythonOperator(
    task_id='phy_store_point_transaction_stg2ods_task',
    provide_context = True,
...
phy_store_point_transaction_ods2edw_task = PythonOperator(
    task_id='phy_store_point_transaction_ods2edw_task',
    provide_context = True,
...
postprocess_phy_store_point_transaction_task = PythonOperator(
    task_id = 'postprocess_phy_store_point_transaction_task',
    provide_context = True,
...
phy_store_point_transaction_sensor_task >> has_new_data_branch_task >> phy_store_point_transaction_stg2ods_task
has_new_data_branch_task >> phy_store_point_transaction_nodata_mail_task >> postprocess_phy_store_point_transaction_task
phy_store_point_transaction_stg2ods_task >>  phy_store_point_transaction_ods2edw_task >> postprocess_phy_store_point_transaction_task