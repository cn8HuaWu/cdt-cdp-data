...
src_entity = 'oms_expense_statement'
DAG_NAME = 'oms_expense_statement_dag'
...
lego_hr_subject = Variable.get('lego_hr_subject').strip() + "-EXPRENSE STATEMENT" 
lego_hr_html_content =  "EXPRENSE STATEMENT " + Variable.get('lego_hr_html_content').strip() 
lego_hr_html_content_premail =  "EXPRENSE STATEMENT Starts running" 
...
def b_choice(**kwargs):
    updated_flag = kwargs['task_instance'].xcom_pull(key='new_data_flag', task_ids='expense_statement_sensor_task')
    if ( updated_flag in('', 'False') ):
        return "postprocess_expense_statement_task"
    else:
...
def mail_or_not(**kwargs):
    ...
    if hour >= hr_work_time:
        return "expense_statement_email_task"
    else:
        return "postprocess_expense_statement_task"
...
def premail_or_not(**kwargs):
    hour = datetime.now().hour
    if hour >= hr_work_time:
        return "expense_statement_premail_task"
    else:
        return "oms_expense_statement_stg2ods_task"
...
args = {
    ...
    'retries': 0,
    "src_name": SRC_NAME,
    "batch_date_args": 'oms_expense_statement_stg2ods_task',
}
...
expense_statement_sensor_task = DatahubSensor(
    mydb=db, 
    dh_id=src_entity, 
    myutil= myutil, 
    task_id='expense_statement_sensor_task', 
    poke_interval=120, 
    dag=dag
)
...
expense_statement_premail_task = EmailOperator(
    task_id="expense_statement_premail_task",
    dag=dag,
...
oms_expense_statement_stg2ods_task = PythonOperator(
    task_id='oms_expense_statement_stg2ods_task',
    provide_context = True,
...
oms_expense_statement_ods2edw_task = PythonOperator(
    task_id='oms_expense_statement_ods2edw_task',
    provide_context = True,
...
expense_statement_email_task = EmailOperator(
    task_id="expense_statement_email_task",
    dag=dag,
    on_failure_callback = dag_failure_handler,
    to = lego_hr_mail_to,
    subject = lego_hr_subject,
...
expense_statement_sync_2_rds_task = SubDagOperator(
    task_id='expense_statement_sync_2_rds_task',
    subdag=sync_subdag(DAG_NAME, 'expense_statement_sync_2_rds_task', myutil, entity_conf, args, entity),
    default_args=args,
...
postprocess_expense_statement_task = PythonOperator(
    task_id = 'postprocess_expense_statement_task',
    provide_context = True,
...
expense_statement_sensor_task >> check_datahub_branch_task >> premail_or_not_branch_task >> oms_expense_statement_stg2ods_task >> oms_expense_statement_ods2edw_task >> expense_statement_sync_2_rds_task>> mail_or_not_branch_task >> expense_statement_email_task >> postprocess_expense_statement_task
premail_or_not_branch_task >> expense_statement_premail_task >> oms_expense_statement_stg2ods_task
mail_or_not_branch_task >> postprocess_expense_statement_task
check_datahub_branch_task >> postprocess_expense_statement_task